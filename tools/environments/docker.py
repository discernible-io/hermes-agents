...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # Egress posture and docker_extra_args each get their own label: env/CA
        # mounts and host mappings (``--add-host``) are immutable after creation —
        # reusing a pre-egress / stale-pin container would bypass the firewall or
        # keep a dead SMTP host mapping.
        ...
        reused = persist_across_processes and self._attach_existing_container(
            task_label, profile_name, egress_label, extra_args_label, network)
        if not reused:
    ...
    def _attach_existing_container(
        self, task_label, profile_name, egress_label, extra_args_label, network: bool,
    ) -> bool:
    ...
    def _find_reusable_container(
        ...
        egress posture / docker_extra_args fingerprint, or ``None`` on miss or any failure.
        The egress posture is a label FILTER for every posture, "off" included: a container
        built with egress on must not be reused after ``hermes egress disable`` (baked-in
        proxy env and CA mounts), and every container this class creates carries the label.
        Extra-args fingerprint is always required so a stale SMTP ``--add-host`` pin (or
        other immutable run flags) is not silently reused. The ``{{.Label "key"}}`` template
